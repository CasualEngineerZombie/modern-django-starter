...
class TestDjangoProjectGenerator:
    ...
    def test_settings_files_have_correct_content(self):
        ...
        assert (
            "PROJECT = 'test_project'" in base_content or 'PROJECT = "test_project"' in base_content
        )

        dev_content = (
            self.project_dir / 'test_project' / 'settings' / 'development.py'
        ).read_text()
        assert (
            "PROJECT = 'test_project'" in dev_content or 'PROJECT = "test_project"' in dev_content
        )

        prod_content = (
            self.project_dir / 'test_project' / 'settings' / 'production.py'
        ).read_text()
        assert (
            "PROJECT = 'test_project'" in prod_content or 'PROJECT = "test_project"' in prod_content
        )
